...
num_diagnosis_cancer = alt.Chart(Q2).mark_bar().encode(
  x = alt.X("tissue_or_organ_of_origin:N", sort='-y',title='Site of cancer origin'), 
...
death_age_distribution = alt.Chart(Q4).mark_boxplot(opacity = 0.8).encode(
    ...
    color = alt.Color("tissue_or_organ_of_origin:N", title = "Site of cancer origin")
).properties(title="Distribution of Age of Death")
...
num_relatives_cancer2 = num_relatives_cancer_base.mark_bar().encode(
    ...
    tooltip = [alt.Tooltip("relationship_primary_diagnosis",title='Relationship with the primary diagnosed patient'), alt.Tooltip("relationship_type",title='Relationship type'),alt.Tooltip("count(relationship_type)",title='Count of records')]
).transform_filter(
    select_cancer
)
